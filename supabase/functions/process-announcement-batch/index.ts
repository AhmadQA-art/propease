...
serve(async (req)=>{
  ...
  try {
    // Parse request body
    const { jobId, batchSize = DEFAULT_BATCH_SIZE } = await req.json();
    // Validate request
    if (!jobId) {
      return errorResponse(400, 'Job ID is required');
    }
    // Fetch job details
    const { data: job, error: jobError } = await supabase.from('announcement_jobs').select('*').eq('id', jobId).single();
    if (jobError || !job) {
    ...
    const { data: bgTask, error: bgTaskError } = await supabase.from('announcement_background_tasks').select('*').eq('job_id', jobId).order('created_at', {
      ascending: false
    }).limit(1).single();
    if (bgTaskError) {
    ...
    const { data: announcement, error: annError } = await supabase.from('announcements').select('title, content, communication_method, type, organization_id').eq('id', bgTask.announcement_id).single();
    ...
    const { data: uniqueTenantContacts, error: tenantsError } = await supabase.rpc('get_announcement_tenant_contacts', {
    ...
    await processTenantBatch(currentBatch, announcement, messagesSent, messagesFailures);
    ...
    return successResponse({
      success: true,
      message: remainingCount > 0 ? 'Batch processed, more batches pending' : 'All batches completed, job finished',
      announcement: {
