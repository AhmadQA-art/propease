...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: tenant } = await supabase.from('tenants').select('id, first_name, last_name').eq('whatsapp_number', normalizedPhone).single();
    if (tenant) {
      // Log the message
      ...
      const { data: tenantByPhone } = await supabase.from('tenants').select('id, first_name, last_name').eq('phone_number', normalizedPhone).single();
      if (tenantByPhone) {
