...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: announcement, error: annError } = await supabase.from('announcements').select('title, content, communication_method, type, organization_id').eq('id', announcementId).single();
    if (annError || !announcement) {
      console.error('Error fetching announcement:', annError);
      return errorResponse(404, 'Announcement not found');
    }
    // Fetch targets
    const { data: targets, error: targetError } = await supabase.from('announcement_targets').select('target_type, target_id, property_id, target_name').eq('announcement_id', announcementId);
    if (targetError) {
    ...
    for (const target of targets){
      if (target.target_type === 'property' && target.property_id) {
        ...
        const { data: units, error: unitsError } = await supabase.from('units').select('id').eq('property_id', target.property_id);
        if (unitsError) {
        ...
        const { data: leases, error: leasesError } = await supabase.from('leases').select('tenant_id').eq('status', 'Active').in('unit_id', units.map((unit)=>unit.id));
        if (leasesError) {
        ...
        const { data: tenants, error: tenantsError } = await supabase.from('tenants').select('id, email, phone_number, whatsapp_number, first_name, last_name').in('id', leases.map((lease)=>lease.tenant_id));
        if (tenantsError) {
        ...
        const { data: tenant, error: tenantError } = await supabase.from('tenants').select('id, email, phone_number, whatsapp_number, first_name, last_name').eq('id', target.target_id).single();
        if (tenantError) {
    ...
    const { data: job, error: jobError } = await supabase.from('announcement_jobs').insert({
    ...
    const messagesSent = [];
    const messagesFailures = [
      ...initialErrors
    ];
    // Process first batch of tenants
    await processTenantBatch(firstBatch, announcement, messagesSent, messagesFailures);
    // Update job with first batch results
    await supabase.from('announcement_jobs').update({
    ...
    if (remainingTenants > 0) {
      ...
      await supabase.from('announcement_background_tasks').insert({
        job_id: job.id,
        announcement_id: announcementId,
    ...
    return successResponse({
      success: true,
      message: remainingTenants > 0 ? 'Announcement sending started (processing in background)' : 'Announcement sent successfully',
      announcement: {
        id: announcementId,
...
async function processTenantBatch(tenants, announcement, messagesSent, messagesFailures) {
